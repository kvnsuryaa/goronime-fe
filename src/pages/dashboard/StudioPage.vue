<template>
  <div class="row">
    ...
    <el-dialog v-model="visibleDialog" title="Create Studio" width="500">
      <el-form :model="payload" :label-position="'top'">
        <el-form-item label="Studio name">
    ...
    <el-dialog v-model="visibleDialogUpdate" title="Update Studio" width="500">
      <el-form :model="payload" :label-position="'top'">
        <el-form-item label="Studio name">
          <el-input v-model="payload.name" autocomplete="off" placeholder="" />
        </el-form-item>
...
const payload = studioStore.payload
const listGenre = computed(() => studioStore.list)
onMounted(async () => {
  await init()
})
...
async function submitData(action: string) {
  if (action === 'update') {
    await studioStore.updateStudio(id.value)
  } else {
    await studioStore.createStudio()
  }
...
async function deleteData(id: string) {
  await studioStore.deleteStudio(id)
  await init()
}
